...
def validate(repo_root: Path, *, verbose: bool = False) -> int:
    ...
    # --- Agent workspace sync validation ---
    claude_agents_dir = repo_root / ".claude" / "agents"
    pb_agents_dir = repo_root / ".patchboard" / "agents"

    if claude_agents_dir.exists() and pb_agents_dir.exists():
        # Build set of patchboard workspace names
        pb_workspace_names = set()
        for d in sorted(pb_agents_dir.iterdir()):
            if d.is_dir() and (d / "index.md").exists():
                pb_workspace_names.add(d.name)

        # Check each .claude/agents/*.md has a matching patchboard workspace
        for md_file in sorted(claude_agents_dir.glob("*.md")):
            stem = md_file.stem  # e.g. "pb-engineer", "engineering-manager"
            # Convention: strip "pb-" prefix to get workspace name
            ws_name = stem[3:] if stem.startswith("pb-") else stem

            if ws_name not in pb_workspace_names:
                errors.append(
                    f"agent-sync: .claude/agents/{md_file.name} has no matching "
                    f".patchboard/agents/{ws_name}/ workspace"
                )
