...
REPO_ROOT="$(git -C "$SCRIPT_DIR" rev-parse --show-toplevel 2>/dev/null || dirname "$(dirname "$SCRIPT_DIR")")"
...
cmd_status() {
    ...
    local agent_pids
    agent_pids=$(pgrep -f "agent_bot.sh|patchboard.bash auto" 2>/dev/null | grep -v "$$" || true)
...
cmd_help() {
    ...
    table_row "list" "List recent sessions"
    ...
    echo ""
    echo -e "    ${DIM}# Start a specific session${NC}"
