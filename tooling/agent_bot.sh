...
Interactive agent session bot. Discovers queued self-hosted sessions
...
Options:
  ...
  --mode poll|select         Session discovery mode (default: interactive prompt)
                               poll   - continuously poll for queued sessions
                               select - pick from queued/recent sessions, then exit
  --poll-interval SECONDS    Poll interval for poll mode (default: 60, skips prompt)
...
Session modes:
  poll   - Watch loop: continuously polls for queued sessions and processes
           them automatically. Good for unattended/daemon operation.
  select - Session picker: shows queued and recent sessions in a menu,
           lets you choose one to execute, then exits. Good for testing
           and targeted work.
...
Examples:
  ...
  agent_bot.sh --cli claude --mode select             # Pick a session to run
EOF
}
...
select_model() {
    ...
    if [[ "$SELECTED_CLI" == "claude" ]]; then
        ...
        echo "  1) claude-sonnet-4.6 (faster, recommended)"
        echo "  2) claude-opus-4.6 (more capable)"
...
select_session_mode() {
    if [[ -n "$SESSION_MODE" ]]; then
        ...
        case "$SESSION_MODE" in
            poll|select) ;;
            *)
                echo -e "${RED}Error: --mode must be 'poll' or 'select', got '${SESSION_MODE}'${NC}" >&2
                exit 1
                ;;
        esac
    else
        echo -e "${YELLOW}Session discovery mode:${NC}"
        echo "  1) poll   - continuously poll for queued sessions (daemon mode)"
        echo "  2) select - pick a session from a menu, then exit"
        echo ""
        read -p "Choice [1]: " choice

        case "${choice:-1}" in
            2) SESSION_MODE="select" ;;
            *) SESSION_MODE="poll" ;;
        esac
    fi

    echo -e "${GREEN}Session mode: ${SESSION_MODE}${NC}"
    echo ""
}

# ─── Session selector ───────────────────────────────────────────────

# Display all sessions grouped by status, let user pick one or paste a session ID.
# Sets SESSION_PICK_ID to the chosen session ID, or empty if cancelled.
# Returns: 0 if session selected, 1 if cancelled/no sessions
select_session_interactive() {
    SESSION_PICK_ID=""

    # --- Status filter prompt ---
    local all_statuses=("queued" "active" "failed" "completed" "stopped")
    local status_labels=("1) queued" "2) active" "3) failed" "4) completed" "5) stopped")

    echo ""
    echo -e "${YELLOW}Which statuses do you want to see?${NC}"
    echo -e "  ${GREEN}${status_labels[0]}${NC}  ${CYAN}${status_labels[1]}${NC}  ${RED}${status_labels[2]}${NC}  ${BLUE}${status_labels[3]}${NC}  ${YELLOW}${status_labels[4]}${NC}"
    echo -e "  ${BOLD}a) all${NC}  ${BOLD}Enter) queued only${NC}"
    read -p "Choose (e.g. 1,2 or a): " status_choice

    local -a selected_statuses=()
    if [[ -z "$status_choice" ]]; then
        selected_statuses=("queued")
    elif [[ "$status_choice" == "a" || "$status_choice" == "A" ]]; then
        selected_statuses=("${all_statuses[@]}")
    else
        IFS=',' read -ra picks <<< "$status_choice"
        for p in "${picks[@]}"; do
            p=$(echo "$p" | tr -d ' ')
            if [[ "$p" =~ ^[1-5]$ ]]; then
                selected_statuses+=("${all_statuses[$(( p - 1 ))]}")
            fi
        done
        if [[ ${#selected_statuses[@]} -eq 0 ]]; then
            echo -e "${YELLOW}[select] Invalid choice, defaulting to queued.${NC}"
            selected_statuses=("queued")
        fi
    fi

    echo -e "${BLUE}[select] Filtering: ${selected_statuses[*]}${NC}"

    # --- Session display + selection loop (supports 'r' to refresh) ---
    while true; do
        echo -e "${BLUE}[select] Discovering sessions...${NC}"
        ensure_on_main
        git -C "$REPO_ROOT" pull --rebase --quiet 2>/dev/null || true

        local all_json
        all_json=$("${SCRIPT_DIR}/agent-list.sh" --all --json 2>/dev/null || echo "[]")

        echo ""
        local idx=1
        local -a session_ids=()

        local status_colors_map
        declare -A status_colors_map=( ["queued"]="${GREEN}" ["active"]="${CYAN}" ["failed"]="${RED}" ["completed"]="${BLUE}" ["stopped"]="${YELLOW}" )

        for st in "${selected_statuses[@]}"; do
            local color="${status_colors_map[$st]}"
            local group_json
            group_json=$(echo "$all_json" | jq --arg st "$st" '[.[] | select(.status == $st)]')

            if [[ "$st" == "queued" ]]; then
                group_json=$(echo "$group_json" | jq 'sort_by(.started_at)')
            else
                group_json=$(echo "$group_json" | jq 'sort_by(.updated_at) | reverse')
            fi
            ...
            echo -e "${color}${st} (${group_count}):${NC}"

            local i=0
            while [[ $i -lt $group_count ]]; do
                local sid tasks workspace prompt
                sid=$(echo "$group_json" | jq -r ".[$i].session_id")
                tasks=$(echo "$group_json" | jq -r "if .[$i].task_ids then (.[$i].task_ids | join(\",\")) elif .[$i].task_id then .[$i].task_id else \"\" end")
                workspace=$(echo "$group_json" | jq -r ".[$i].workspace_id // \"—\"")
                prompt=$(echo "$group_json" | jq -r ".[$i].prompt // \"\"" | tr '\n' ' ')
                prompt="${prompt:0:55}"

                printf "  ${CYAN}%2d)${NC} %-16s  %-12s  %-14s  %s\n" \
                    "$idx" "$sid" "$tasks" "$workspace" "$prompt"
        ...
        if [[ "$total" -eq 0 ]]; then
            echo -e "${YELLOW}[select] No sessions found for: ${selected_statuses[*]}${NC}"
        fi

        # Prompt for selection
        if [[ "$total" -gt 0 ]]; then
            echo -e "${YELLOW}Enter a number (1-${total}), a session ID, 'r' to refresh, or 'q' to cancel:${NC}"
        else
            echo -e "${YELLOW}Enter a session ID, 'r' to refresh, or 'q' to cancel:${NC}"
        fi
        read -p "Selection: " selection

        if [[ "$selection" == "q" || "$selection" == "Q" ]]; then
            echo -e "${YELLOW}[select] Cancelled.${NC}"
            return 1
        fi

        if [[ "$selection" == "r" || "$selection" == "R" ]]; then
            echo -e "${BLUE}[select] Refreshing...${NC}"
            echo ""
            continue
        fi
        ...
        if [[ "$selection" =~ ^[0-9]+$ ]] && [[ "$selection" -ge 1 && "$selection" -le "$total" ]]; then
            SESSION_PICK_ID="${session_ids[$(( selection - 1 ))]}"
        else
            # Treat as a session ID — verify the file exists
            local candidate_file="${SESSION_DIR}/${selection}.json"
            if [[ -f "$candidate_file" ]]; then
                SESSION_PICK_ID="$selection"
            else
                echo -e "${RED}[select] No session found for '${selection}' (not a valid number or session ID).${NC}" >&2
                return 1
            fi
        fi

        echo -e "${GREEN}[select] Selected: ${SESSION_PICK_ID}${NC}"
        echo ""
        return 0
    done
}
...
show_permissions_and_confirm() {
    ...
    if [[ "$SELECTED_CLI" == "claude" ]]; then
        echo -e "Claude will run with ${BOLD}--dangerously-skip-permissions${NC}"
        echo ""
        ...
        echo "  Sub-agent spawning"
        echo ""
        echo -e "${YELLOW}Note: This is required for sub-agent orchestration workflows.${NC}"
    else
...
# Ensure session uses subdir layout, migrating flat file if needed.
# Prints the path to session.json inside the subdir.
ensure_session_dir() {
    local sid="$1"
...
# Persist the stdout transcript after agent run completes.
# Moves the temp file into the session subdir and generates a summary.
persist_transcript() {
    local session_id="$1"
    ...
    generate_transcript_summary "${session_subdir}/transcript.jsonl" "${session_subdir}/summary.txt"

    # Compress raw JSONL to save space (~10:1 compression)
    gzip "${session_subdir}/transcript.jsonl" 2>/dev/null || true
}

# Extract human-readable text from JSONL transcript using jq.
generate_transcript_summary() {
...

    {
        ...
        jq -rj 'select(.type == "stream_event" and .event.delta?.type? == "text_delta") | .event.delta.text // empty' "$jsonl_file" 2>/dev/null || true

        echo ""
        echo "=== Tool Usage ==="
        # Extract tool use markers
        jq -r 'select(.type == "stream_event" and .event.type? == "content_block_start" and .event.content_block?.type? == "tool_use") | "- " + (.event.content_block.name // "unknown")' "$jsonl_file" 2>/dev/null || true

        echo ""
        echo "=== Final Result ==="
        # Extract the final result
        jq -rj 'select(.type == "result") | .result // empty' "$jsonl_file" 2>/dev/null || true
    } > "$output_file"
}
...
4. Do not stop to ask 'Would you like me to...?' — just do it"
    ...
    if [[ "$SELECTED_CLI" == "claude" ]]; then
        if [[ "$NON_INTERACTIVE" == "true" ]]; then
            ...
            wait 2>/dev/null  # ensure tee/jq subprocesses flush to disk
            set -eo pipefail
            ...
            # Interactive mode: no pipes (tee breaks TTY), no -c (resumes last conversation).
            # Pass prompt as positional arg to start a fresh conversation.
        ...
        if [[ "$NON_INTERACTIVE" == "true" ]]; then
            ...
            # Interactive mode: no pipes (breaks TTY)
            $timeout_cmd copilot --model "$SELECTED_MODEL" \
                --allow-all-tools \
                "$prompt"
            agent_exit=$?
        ...
        if [[ -n "$session_id" ]]; then
            ...
            if claim_session "$session_id"; then
                ...
                if [[ -s "$stdout_tmp" ]]; then
                    ...
                    # Persist transcript (moves temp file to session subdir + gzips)
...
# ─── Select mode (pick one session and run) ──────────────────────────

select_mode_run() {
    echo -e "${BLUE}═══════════════════════════════════════════${NC}"
    echo -e "${GREEN}Session selector${NC}  (v${PATCHBOARD_TOOLS_VERSION})"
    echo -e "   CLI: ${SELECTED_CLI}"
    echo -e "   Model: ${SELECTED_MODEL}"
    echo -e "   Mode: $(if [[ "$NON_INTERACTIVE" == "true" ]]; then echo "non-interactive"; else echo "interactive"; fi)"
    if [[ "${AGENT_TIMEOUT:-0}" -gt 0 ]]; then
    ...
    if ! select_session_interactive; then
        echo -e "${YELLOW}No session selected. Exiting.${NC}"
        return
    fi

    local session_id="$SESSION_PICK_ID"
    local session_file
    session_file=$(ensure_session_dir "$session_id")

    # Check session status — only queued sessions can be claimed
    local status
    ...
    if [[ "$status" == "queued" ]]; then
        # Claim and run
        if ! claim_session "$session_id"; then
            echo -e "${RED}Failed to claim session ${session_id}.${NC}" >&2
            return
        fi
    elif [[ "$status" == "active" || "$status" == "failed" || "$status" == "completed" ]]; then
        # Non-queued session — confirm before running
        echo -e "${YELLOW}Session ${session_id} has status '${status}'.${NC}"
        echo -e "${YELLOW}This will re-run the session prompt without claiming or updating status.${NC}"
        ...
        echo -e "${RED}Session ${session_id} has unexpected status '${status}'. Aborting.${NC}" >&2
        return
    fi
    ...
    # Only do post-run verification if we claimed the session (it was queued)
    if [[ "$status" == "queued" ]]; then
        local session_handled=false
        if post_run_verify "$session_id" "$rc" "$stderr_content" "$stdout_content"; then
            session_handled=true
...
main() {
    ...
    select_model
    select_session_mode
    if [[ "$SESSION_MODE" == "poll" ]]; then
        select_poll_interval
    fi
    select_mode
    show_permissions_and_confirm

    if [[ "$SESSION_MODE" == "select" ]]; then
        select_mode_run
    else
        watch_loop
    fi
